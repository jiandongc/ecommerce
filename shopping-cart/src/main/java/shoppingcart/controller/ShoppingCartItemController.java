...
public class ShoppingCartItemController {
    ...
    public ResponseEntity<CartSummary> createCartItem(@PathVariable UUID cartUid, @RequestBody ShoppingCartItem cartItem){
        try{
            final ShoppingCart cart = cartItemService.createCartItem(cartUid, cartItem);
            return new ResponseEntity<>(cartSummaryMapper.map(cart), CREATED);
    ...
    public ResponseEntity<CartSummary> deleteCartItem(@PathVariable UUID cartUid, @PathVariable String sku){
        ...
        return new ResponseEntity<>(cartSummaryMapper.map(cart), OK);
    }
